...
echo "Starting CRM setup with local code..."
...
if [ -f "frappe-bench/apps/frappe/frappe/__init__.py" ]; then
    ...
    # Get Frappe (will download since we don't have it locally)
    bench get-app frappe https://github.com/frappe/frappe.git --branch develop
    ...
    # Install apps - use local CRM code that is mounted
    echo "Installing local CRM app..."
    bench --site crm.localhost install-app crm --skip-assets
